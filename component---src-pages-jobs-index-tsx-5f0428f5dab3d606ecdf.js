;(window.webpackJsonp = window.webpackJsonp || []).push([
  [9],
  {
    158(e, a, t) {
      t.r(a)
      const n = t(0)
      const A = t.n(n)
      const i = t(261)
      const r = t(193)
      const s = t(175)
      const c = t(176)
      const o = t(184)
      a.default = function() {
        return A.a.createElement(
          o.a,
          null,
          A.a.createElement(r.a, { title: 'Jobs' }),
          A.a.createElement(s.a, null),
          A.a.createElement(c.a, { name: 'job' }),
          A.a.createElement(i.a, null)
        )
      }
    },
    171(e, a, t) {
      t(39)
      const n = t(95)
      const A = t.n(n)
      const i = t(8)
      const r = t.n(i)
      const s = t(0)
      const c = t.n(s)
      const o = t(166)
      const l = t.n(o)
      const m = t(185)
      const p = t.n(m)
      const C = (function(e) {
        function a() {
          return e.apply(this, arguments) || this
        }
        return (
          r()(a, e),
          (a.prototype.render = function() {
            let e
            let a
            const t = this.props
            const n = t.color
            const i = t.onClick
            const r = t.children
            const s = t.isDisabled
            const o = t.className
            const m = t.type
            const C = t.isShadow
            const E = t.href
            const d = A()(t, ['color', 'onClick', 'children', 'isDisabled', 'className', 'type', 'isShadow', 'href'])
            const u = p.a[n]
            return E
              ? c.a.createElement(
                  'a',
                  Object.assign({ className: l()(p.a.btn, u, o, ((a = {}), (a[p.a.shadow] = C), a)), onClick: i, href: E }, d),
                  r
                )
              : c.a.createElement(
                  'button',
                  Object.assign({ disabled: s, onClick: i, type: m, className: l()(p.a.btn, u, o, ((e = {}), (e[p.a.shadow] = C), e)) }, d),
                  r
                )
          }),
          a
        )
      })(s.Component)
      ;(C.defaultProps = { color: 'blue', type: 'button', className: '', isDisabled: !1, isShadow: !1, href: '' }), (a.a = C)
    },
    175(e, a, t) {
      const n = t(8)
      const A = t.n(n)
      const i = t(0)
      const r = t.n(i)
      const s = (t(169), t(166))
      const c = t.n(s)
      const o = t(174)
      const l = t(172)
      const m = t(173)
      const p = t(171)
      const C = (t(165), t(170), t(182))
      const E = t(186)
      const d = t.n(E)
      const u = function(e) {
        let a
        let t
        const n = e.isOpen
        const A = e.isFixed
        const i = Object(l.a)()
        const s = i.menuLinks
        const o = i.contact
        const m = A ? ['black'] : ['white', 'black']
        return r.a.createElement(
          'div',
          { className: c()(d.a.wrap, ((a = {}), (a[d.a.open] = n), a), ((t = {}), (t[d.a.fixed] = A), t)) },
          r.a.createElement(
            'nav',
            { className: d.a.list },
            s.map(function(e) {
              return e.link
                ? r.a.createElement(C.a, { key: e.name, color: m, name: e.name, icon: e.icon, link: e.link })
                : r.a.createElement(C.a, { key: e.name, color: m, name: e.name, icon: e.icon })
            }),
            r.a.createElement(
              p.a,
              { className: d.a.btn, href: `mailto:${o.email}` },
              r.a.createElement('span', { className: d.a.btnText }, 'Apply')
            )
          )
        )
      }
      const g = t(187)
      const f = t.n(g)
      const h = (function(e) {
        function a() {
          for (var a, t = arguments.length, n = new Array(t), A = 0; A < t; A++) n[A] = arguments[A]
          return (
            ((a = e.call.apply(e, [this].concat(n)) || this).state = { isOpen: !1 }),
            (a.changeVisibilityMenu = function() {
              a.setState({ isOpen: !a.state.isOpen })
            }),
            a
          )
        }
        return (
          A()(a, e),
          (a.prototype.render = function() {
            let e
            let a
            let t
            const n = this.state.isOpen
            const A = this.props.isBlack
            return r.a.createElement(
              'div',
              { className: c()(f.a.wrap, ((e = {}), (e[f.a.white] = !A), e), ((a = {}), (a[f.a.black] = A), a)) },
              r.a.createElement(
                'div',
                { className: c()(f.a.wrapHamburger, ((t = {}), (t[f.a.open] = n), t)), onClick: this.changeVisibilityMenu },
                r.a.createElement('div', { className: f.a.inner })
              ),
              r.a.createElement(u, { isOpen: n })
            )
          }),
          a
        )
      })(i.Component)
      const Q = t(188)
      const w = t.n(Q)
      const B = function(e) {
        let a
        const t = e.isFixed
        const n = Object(l.a)().contact
        return r.a.createElement(
          'div',
          { className: c()(w.a.container, ((a = {}), (a[w.a.fixed] = t), a)) },
          r.a.createElement(
            'div',
            { className: w.a.wrap },
            r.a.createElement(
              'div',
              { className: w.a.leftBox },
              r.a.createElement(o.a, { to: '/', className: w.a.wrapLogo }, r.a.createElement(m.a, { name: 'optimax' })),
              r.a.createElement(u, { isFixed: t })
            ),
            r.a.createElement(
              'div',
              { className: w.a.rightBox },
              r.a.createElement('div', { className: w.a.wrapLanguage }, r.a.createElement(m.a, { name: 'language' })),
              r.a.createElement(
                p.a,
                { className: w.a.btn, color: t ? 'blue' : 'white', href: `mailto:${n.email}` },
                r.a.createElement('span', { className: w.a.btnText }, 'Apply')
              )
            ),
            r.a.createElement(h, { isBlack: t })
          )
        )
      }
      const v = t(189)
      const I = t.n(v)
      const O = (function(e) {
        function a() {
          for (var a, t = arguments.length, n = new Array(t), A = 0; A < t; A++) n[A] = arguments[A]
          return (
            ((a = e.call.apply(e, [this].concat(n)) || this).state = { topRect: 0, isFixed: !1 }),
            (a.calcHeight = function() {
              a.setState({ topRect: a.wrapRef.current.getBoundingClientRect().top + window.pageYOffset }, a.onSticky())
            }),
            (a.onSticky = function() {
              const e = window.pageYOffset
              const t = a.state.topRect + a.props.topOffset
              e <= t ? (a.setState({ isFixed: !1 }), a.props.isFixed(!1)) : e >= t && (a.setState({ isFixed: !0 }), a.props.isFixed(!0))
            }),
            (a.wrapRef = r.a.createRef()),
            a
          )
        }
        A()(a, e)
        const t = a.prototype
        return (
          (t.componentDidMount = function() {
            window.addEventListener('scroll', this.onSticky)
          }),
          (t.componentWillUnmount = function() {
            window.removeEventListener('scroll', this.onSticky)
          }),
          (t.render = function() {
            let e
            const a = this.props.children
            const t = this.state.isFixed
            return r.a.createElement('div', { className: c()(I.a.wrap, ((e = {}), (e[I.a.fixed] = t), e)), ref: this.wrapRef }, a)
          }),
          a
        )
      })(i.Component)
      O.defaultProps = {
        topOffset: 0,
        isFixed() {
          return !1
        }
      }
      const N = O
      const x = t(190)
      const F = t.n(x)
      const G = (function(e) {
        function a() {
          for (var a, t = arguments.length, n = new Array(t), A = 0; A < t; A++) n[A] = arguments[A]
          return (
            ((a = e.call.apply(e, [this].concat(n)) || this).state = { isFixed: !1 }),
            (a.isFixed = function(e) {
              a.setState({ isFixed: e })
            }),
            a
          )
        }
        return (
          A()(a, e),
          (a.prototype.render = function() {
            const e = this.state.isFixed
            return this.props.isHome
              ? r.a.createElement(N, { isFixed: this.isFixed }, r.a.createElement(B, { isFixed: e }))
              : r.a.createElement(
                  'div',
                  { className: F.a.wrap },
                  r.a.createElement('div', { className: F.a.sticky }, r.a.createElement(B, { isFixed: !0 }))
                )
          }),
          a
        )
      })(i.Component)
      G.defaultProps = { isHome: !1 }
      a.a = G
    },
    176(e, a, t) {
      t(165)
      const n = t(0)
      const A = t.n(n)
      const i = t(177)
      const r = t.n(i)
      const s = t(178)
      const c = t.n(s)
      const o = t(179)
      const l = t.n(o)
      const m = t(180)
      const p = t.n(m)
      const C = t(195)
      const E = t.n(C)
      const d = { glassesusa: r.a, ottica: c.a, uvp: l.a, job: p.a }
      a.a = function(e) {
        const a = e.name
        const t = e.children
        const n = e.img
        const i = n || d[a]
        return void 0 === i
          ? (console.error('Requested icon does not exist', a), null)
          : A.a.createElement(
              'div',
              { className: E.a.wrap },
              A.a.createElement(
                'div',
                { className: E.a.inner },
                t && A.a.createElement('div', { className: E.a.content }, t),
                A.a.createElement('div', { className: E.a.wrapImg }, A.a.createElement('img', { className: E.a.img, src: i, alt: '' }))
              )
            )
      }
    },
    177(e, a, t) {
      e.exports = `${t.p}static/glassesusaD-df87da52bd1aac9956ef0bb205d92854.jpg`
    },
    178(e, a, t) {
      e.exports = `${t.p}static/otticaD-db12817567b79be72a8656a8a56d46b4.jpg`
    },
    179(e, a, t) {
      e.exports = `${t.p}static/uvpD-9bcdf3f21886040ca3103b6c6f65abc3.jpg`
    },
    180(e, a, t) {
      e.exports = `${t.p}static/job-3b2255e609e68a0b82c2cb7d9e303f4f.jpg`
    },
    181(e, a, t) {
      const n = t(8)
      const A = t.n(n)
      const i = t(0)
      const r = t.n(i)
      const s = t(166)
      const c = t.n(s)
      const o = t(233)
      const l = t.n(o)
      const m = (function(e) {
        function a() {
          return e.apply(this, arguments) || this
        }
        return (
          A()(a, e),
          (a.prototype.render = function() {
            let e
            const a = this.props
            const t = a.children
            const n = a.isWhite
            return r.a.createElement('h2', { className: c()(l.a.title, ((e = {}), (e[l.a.white] = n), e)) }, t)
          }),
          a
        )
      })(i.Component)
      a.a = m
    },
    192(e, a, t) {
      const n = t(8)
      const A = t.n(n)
      const i = t(0)
      const r = t.n(i)
      const s = t(166)
      const c = t.n(s)
      const o = t(254)
      const l = t.n(o)
      const m = (function(e) {
        function a() {
          return e.apply(this, arguments) || this
        }
        return (
          A()(a, e),
          (a.prototype.render = function() {
            let e
            const a = this.props
            const t = a.isWhite
            const n = a.children
            return r.a.createElement('h3', { className: c()(l.a.subtitle, ((e = {}), (e[l.a.white] = t), e)) }, n)
          }),
          a
        )
      })(i.Component)
      a.a = m
    },
    193(e, a, t) {
      const n = t(0)
      const A = t.n(n)
      const i = t(222)
      const r = t.n(i)
      const s = t(194)
      const c = t.n(s)
      const o = t(172)
      const l = function(e) {
        const a = Object(o.a)()
        const t = e.title || a.title
        return A.a.createElement(r.a, {
          htmlAttributes: { lang: 'en' },
          title: t,
          titleTemplate: `%s - ${a.title}`,
          link: [{ rel: 'shortcut icon', type: 'image/png', href: `${c.a}` }]
        })
      }
      ;(l.defaultProps = { lang: 'en', meta: [], keywords: [] }), (a.a = l)
    },
    194(e, a) {
      e.exports =
        'data:image/png;base64,'
    },
    209(e, a, t) {
      const n = t(210)
      a.a = function() {
        return n.data.allMarkdownRemark.edges
      }
    },
    210(e) {
      e.exports = {
        data: {
          allMarkdownRemark: {
            edges: [
              {
                node: {
                  excerpt:
                    'Optimax investment Ltd - существуем с 2008 года и известны на рынке США, как один из ведущих Интернет-магазинов по реализации оптики мировых…',
                  id: '173c5275-5f9d-5550-b465-ab9e10ba4177',
                  frontmatter: {
                    title: 'QA engineer (тестирование)',
                    address: 'Tolyatti, St. 8b',
                    date: '2018-11-28T05:14:54.000Z',
                    path: '/jobs/qa'
                  }
                }
              },
              {
                node: {
                  excerpt:
                    'Ищем в свою растущую команду front-end-разработчика. Наша философия — сделать наш код простым, лаконичным и легко тестируемым. Мы постоянно…',
                  id: '5e1994e1-252a-516a-95c6-a548d311ff3a',
                  frontmatter: {
                    title: 'Frontend-разработчик',
                    address: 'Tolyatti, St. 8b',
                    date: '2018-11-18T02:33:46.000Z',
                    path: '/jobs/frontend'
                  }
                }
              },
              {
                node: {
                  excerpt:
                    'Готовы взять в свою дружную it-команду Программиста в отдел Devops.Optimax investment Ltd - существуем с 2008 года и известны на рынке США…',
                  id: 'e0f35af8-dee8-5892-82ed-2921734549c8',
                  frontmatter: {
                    title: 'Системный администратор Linux (отдел Devops)',
                    address: 'Tolyatti, St. 8b',
                    date: '2018-11-18T02:33:46.000Z',
                    path: '/jobs/devops'
                  }
                }
              }
            ]
          }
        }
      }
    },
    261(e, a, t) {
      const n = t(0)
      const A = t.n(n)
      const i = t(181)
      const r = t(192)
      const s = t(174)
      const c = t(173)
      const o = t(285)
      const l = t.n(o)
      const m = function(e) {
        const a = e.item
        return A.a.createElement(
          s.a,
          { to: a.path, className: l.a.wrap },
          A.a.createElement(
            'div',
            { className: l.a.content },
            A.a.createElement('span', { className: l.a.title }, a.title),
            A.a.createElement('span', { className: l.a.address }, a.address),
            A.a.createElement('span', { className: l.a.text }, a.text)
          ),
          A.a.createElement('div', { className: l.a.wrapArrow }, A.a.createElement(c.a, { name: 'arrow' }))
        )
      }
      const p = t(209)
      const C = t(286)
      const E = t.n(C)
      a.a = function() {
        const e = Object(p.a)()
        return A.a.createElement(
          'div',
          { className: E.a.wrap },
          A.a.createElement(
            'div',
            { className: E.a.wrapTitles },
            A.a.createElement(i.a, null, 'Join Us'),
            A.a.createElement(r.a, null, 'Current openings')
          ),
          e.map(function(e) {
            return A.a.createElement(m, { key: e.node.frontmatter.path, item: e.node.frontmatter })
          })
        )
      }
    }
  }
])
// # sourceMappingURL=component---src-pages-jobs-index-tsx-5f0428f5dab3d606ecdf.js.map
